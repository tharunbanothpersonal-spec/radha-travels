...
(function () {
  ...
  const onScroll = () =>
    (h.style.boxShadow =
      window.scrollY > 4 ? '0 6px 16px rgba(15,23,42,.08)' : 'none');
  onScroll();
...
(function () {
  ...
  function getTextWrapper(fig) {
    return (
      fig.querySelector('.hero-text') || fig.querySelector('.splash-content')
    );
  }
...
(function () {
  ...
  const go = () => {
    ...
    const activeIndex =
      Array.from(bg.querySelectorAll('figure')).findIndex((f) =>
...
function animateActiveHeroText(bg, activeIndex) {
  ...
  const active = bg
    ...
    [activeIndex]?.querySelector('.hero-text');
  if (active) {
...
(function () {
  const openBtns = Array.from(
    document.querySelectorAll('[data-open-estimate]')
  );
...
async function loadPricing() {
  ...
  try {
    ...
    const res = await fetch("/api/pricing?ts=" + Date.now());
    ...
    rows.forEach(row => {
      ...
      if(row.service === "Outstation"){
        ...
        PRICING.Outstation[row.vehicle] = {
          perKm: row.per_km,
      ...
      if(row.service === "Local"){
        ...
        PRICING.Local[row.vehicle] = {
          baseKm: row.base_km,
          baseHours: row.base_hours,
          basePrice: row.base_price,
          perKm: row.per_km,
          extraPerHour: row.extra_per_hour
        };
...
document.addEventListener("DOMContentLoaded", loadPricing);
  ...
  function calcEstimate(e) {
    ...
    if(!PRICING[svc] || !PRICING[svc][seg]){

  alert("Pricing loading... please try again.");
  return;
